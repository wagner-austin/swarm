...
class Shutdown(BaseDIClientCog):
    def __init__(
        ...
        *,
        ...
        import swarm.core.metrics as default_metrics
        from swarm.frontends.discord.discord_interactions import safe_send as default_safe_send
        from swarm.frontends.discord.discord_owner import get_owner as default_get_owner
        ...
        # DI for lifecycle, fallback to discord_bot.lifecycle for production
        self.lifecycle = (
            lifecycle if lifecycle is not None else getattr(discord_bot, "lifecycle", None)
        )

    @app_commands.command(name="shutdown", description="Cleanly shut the swarm down (owner only).")
    ...
    async def _shutdown_impl(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await self.safe_send(interaction, "❌ Could not resolve swarm owner.", ephemeral=True)
            return
        ...
        # 1️⃣ Auxiliary services are shut down by the discord_bot's lifecycle handler.
        ...
        if self.lifecycle is not None:
            ...
            # Fallback: close discord_bot directly (should not happen in production)
...
async def setup(discord_bot: commands.Bot) -> None:
    await discord_bot.add_cog(Shutdown(discord_bot=discord_bot))
