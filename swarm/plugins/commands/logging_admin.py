...
Owner can query and change the root log level without restarting the swarm:
...
class LoggingAdmin(commands.Cog):
    ...
    def __init__(self, discord_bot: commands.Bot) -> None:  # noqa: D401 – simple description
    ...
    async def loglevel(self, interaction: discord.Interaction, level: str | None = None) -> None:  # noqa: D401
        ...
        # Only the swarm owner may adjust log levels.
        try:
        ...
        if interaction.user.id != owner.id:
            await safe_send(
                interaction, "❌ Only the swarm owner can use this command.", ephemeral=True
            )
            return
