...
Currently provides:
    • ``background_app_command`` – run long-running command logic in a detached
      ...
      unhandled exceptions are routed through :pyfunc:`swarm.frontends.discord.discord_interactions.safe_send`.
"""
...
from swarm.frontends.discord.discord_interactions import safe_defer, safe_send
...
async def setup(_bot: commands.Bot) -> None:  # pragma: no cover
    ...
    `swarm.plugins.commands.*` expecting a ``setup`` coroutine.  This file only
