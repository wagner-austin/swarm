...
``list`` sub-commands so swarm operators can manage personas without touching the
filesystem directly.
...
Those helpers mutate the in-memory :pydata:`swarm.ai.personas.PERSONALITIES` dict
and the underlying YAML files located in
:pydata:`swarm.ai.personas._CUSTOM_DIR`.
"""
...
from swarm.ai.personas import (  # noqa: F401 – re-export for tests
    _CUSTOM_DIR,
    PERSONALITIES,
    Persona,
...
class PersonaAdmin(commands.GroupCog, group_name="persona"):
    ...
    def __init__(
        ...
        from swarm.frontends.discord.discord_interactions import (
    ...
    async def reload_cmd(self, interaction: discord.Interaction) -> None:
        ...
        busy swarms or slow disks).
        """

        await self.safe_defer(interaction, ephemeral=True, thinking=True)

        import swarm.ai.personas as p  # runtime import
    ...
    async def import_cmd(
        ...
        and merged into the in-memory registry without restarting the swarm.
        """
        ...
        try:
            ...
            await safe_send(interaction, "❌ Could not resolve swarm owner.", ephemeral=True)
            return
        ...
        from swarm.ai.personas import _SECRET_FILE, PERSONALITIES
