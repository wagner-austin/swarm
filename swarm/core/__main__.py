...
def _build_parser() -> argparse.ArgumentParser:  # noqa: D401 – imperative style
    ...
    The goal is to let ``python -m swarm.core --help`` exit **immediately**
    ...
    try:
        ...
        version: str = (
            _ilmd.version("swarm") if "swarm" in _ilmd.packages_distributions() else "unknown"
        )
    except Exception:  # pragma: no cover – metadata lookup best-effort
        version = "unknown"

    parser = argparse.ArgumentParser(
        prog="python -m swarm.core",
        add_help=False,  # we add it manually to keep tight control
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=dedent(
            """\
            Swarm AI Task Execution System
            ------------------------------
            Run *without arguments* to start Swarm using environment
...
def cli(argv: list[str] | None = None) -> None:  # noqa: D401
    """Entry-point for ``python -m swarm.core``."""
