...
import pytest
@pytest.mark.asyncio
async def test_serialize_list_empty():
    assert serialize_list([]) == ""

@pytest.mark.asyncio
async def test_serialize_list_non_empty():
    items = ["a", "b", "c"]
...
@pytest.mark.asyncio
async def test_deserialize_list_empty():
    assert deserialize_list("") == []

@pytest.mark.asyncio
async def test_deserialize_list_non_empty():
    serialized = "a,b,c"
    result = deserialize_list(serialized)
    assert result == ["a", "b", "c"]

@pytest.mark.asyncio
async def test_deserialize_list_with_spaces():
    serialized = "a, b, c"
    result = deserialize_list(serialized)
    assert result == ["a", "b", "c"]

@pytest.mark.asyncio
async def test_serialize_deserialize_cycle():
