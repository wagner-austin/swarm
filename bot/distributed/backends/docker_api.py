...
class DockerApiBackend(ScalingBackend):
    ...
    def __init__(
        self,
        image: str = "discord-bot:latest",
        network: str | None = None,
        project_name: str = "discordbot",
    ...
    async def _create_worker_container(self, worker_type: str, instance_num: int) -> bool:
        ...
        try:
            # Environment variables matching docker-compose.yml worker service
            environment = {
                "METRICS_PORT": str(self.worker_metrics_port),
                "DISPLAY": ":99",
                "REDIS_URL": os.environ.get("REDIS_URL"),  # Pass through REDIS_URL from .env
                "LOG_FORMAT": "json",
                "LOG_TO_FILE": "0",
                "PYTHONPATH": "/app",
                "JOB_TYPE_PREFIX": worker_type,  # Critical: Set worker type!
            }
