...
def initialize_and_wire_container(
    ...
    logger.info("Auto-discovering bot.plugins sub-modules for DI wiring …")
    packages_to_wire = []

    try:
        root_pkg = import_module("bot.plugins")
        packages_to_wire.append(root_pkg)

        for mod_info in pkgutil.walk_packages(root_pkg.__path__, "bot.plugins."):
            try:
                packages_to_wire.append(import_module(mod_info.name))
            except ImportError as e:
                logger.error(f"Failed to import {mod_info.name} for DI wiring: {e}")
    except ImportError as e:
        logger.error(f"Could not import bot.plugins package: {e}")
